...
namespace YU.ECS
{
    public class GenerateCubeSystem : ComponentSystem
    {
        ...
        protected override void OnUpdate()
        {
            ...
            //    var entityManager = World.Active.GetOrCreateManager<EntityManager>();
            //    for (int i = 0; i < 400* MusicController.Instance.m_currBeatScore; i++)
            //    {
            ...
            {
                Debug.Log("dddddddddddeeeet");
                var entityManager = World.Active.GetOrCreateManager<EntityManager>();
                //GetComponentGroup(typeof(HealthComponent)).SetFilter<HealthComponent>(new HealthComponent { healthValue = 1 });
                ComponentGroup componentGroup = GetComponentGroup(typeof(HealthComponent));
                entityManager.DestroyEntity(componentGroup);
            }
